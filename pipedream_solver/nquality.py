...
class QualityBuilder():
    ...
    def import_hydraulic_states(self, _dt):  # Previous
    ...
    def import_hydraulic_states_2nd(self, WQ_coeff, _dt):  # Do not re-calculate the Q_dn, Q_up
        self._H_j_next = self.hydraulics.H_j
        self._h_Ik_next = self.hydraulics.h_Ik
        self._Q_ik_next = self.hydraulics.Q_ik
        self._Q_uk_next = self.hydraulics.Q_uk
        ...
        # Calculate the cross-sectional area of links
        self._A_ik_next = WQ_coeff._A_ik_next
        self._A_ik_prev = self.hydraulics._A_ik
        self._Q_ik_up_next = WQ_coeff._Q_ik_up_next
        self._Q_ik_dn_next = WQ_coeff._Q_ik_dn_next
        self._Q_uk_up_next = WQ_coeff._Q_uk_up_next
        self._Q_uk_dn_next = WQ_coeff._Q_uk_dn_next
        self._Q_dk_up_next = WQ_coeff._Q_dk_up_next
        self._Q_dk_dn_next = WQ_coeff._Q_dk_dn_next
        self._A_uk_next = WQ_coeff._A_uk_next
        self._A_uk_prev = WQ_coeff._A_uk_prev
        self._A_dk_next = WQ_coeff._A_dk_next
        self._A_dk_prev = WQ_coeff._A_dk_prev
    ...
    def link_coeffs_2nd(self, WQ_coeff, _dt=None, first_iter=True):
        """
        Compute link momentum coefficients: a_ik, b_ik, c_ik and P_ik.
        """
        # Import instance variables
        ...
        _gamma_dk = gamma_ik(_dt, _c_dk, _A_dk_next, _dx_dk)
        # Export to instance variables
        self._alpha_ik = WQ_coeff._alpha_ik
        self._beta_ik = WQ_coeff._beta_ik
        self._chi_ik = WQ_coeff._chi_ik
        self._gamma_ik = _gamma_ik
        self._alpha_uk = WQ_coeff._alpha_uk
        self._beta_uk = WQ_coeff._beta_uk
        self._chi_uk = WQ_coeff._chi_uk
        self._gamma_uk = _gamma_uk
        self._alpha_dk = WQ_coeff._alpha_dk
        self._beta_dk = WQ_coeff._beta_dk
        self._chi_dk = WQ_coeff._chi_dk
        self._gamma_dk = _gamma_dk
    ...
    def node_coeffs_2nd(self, WQ_coeff, _Q_0Ik=None, _c_0Ik=None, _dt=None, first_iter=True):
        """
        Compute nodal continuity coefficients: D_Ik and E_Ik.
        """
        # Import instance variables
        ...
        if _c_0Ik is None:
            _c_0Ik = np.zeros(_h_Ik_next.size)
        # Compute continuity coefficients
        numba_node_coeffs_2nd(_eta_Ik, _h_Ik_next, _h_Ik_prev, _c_Ik_prev, _c_0Ik, _Q_0Ik, _A_SIk, _K_Ik,
                                  _is_start, _is_end, _kI, _dt)
        # Export instance variables
        self._kappa_Ik = WQ_coeff._kappa_Ik
        self._lambda_Ik = WQ_coeff._lambda_Ik
        self._mu_Ik = WQ_coeff._mu_Ik
        self._eta_Ik = _eta_Ik
    ...
    def forward_recurrence_2nd(self, WQ_coeff):
        """
        Compute forward recurrence coefficients: T_ik, U_Ik, V_Ik, and W_Ik.
        """
        # Import instance variables
        _I_1k = self._I_1k                # Index of first junction in each superlink
        _i_1k = self._i_1k                # Index of first link in each superlink
        _alpha_ik = WQ_coeff._alpha_ik
        _beta_ik = WQ_coeff._beta_ik
        _chi_ik = WQ_coeff._chi_ik
        _gamma_ik = self._gamma_ik
        _kappa_Ik = WQ_coeff._kappa_Ik
        _lambda_Ik = WQ_coeff._lambda_Ik
        _mu_Ik = WQ_coeff._mu_Ik
        _eta_Ik = self._eta_Ik
        _T_ik = WQ_coeff._T_ik                # Recurrence coefficient T_ik
        _U_Ik = WQ_coeff._U_Ik                # Recurrence coefficient U_Ik
        _V_Ik = self._V_Ik                # Recurrence coefficient V_Ik
        _W_Ik = WQ_coeff._W_Ik                # Recurrence coefficient W_Ik
        NK = self.NK
        nk = self.nk
        numba_forward_recurrence_2nd(_T_ik, _U_Ik, _V_Ik, _W_Ik, _alpha_ik, _beta_ik, _chi_ik,
                                 _gamma_ik, _kappa_Ik, _lambda_Ik, _mu_Ik, _eta_Ik,
                                 NK, nk, _I_1k, _i_1k)
        # Export instance variables
        self._T_ik = _T_ik
    ...
    def backward_recurrence_2nd(self, WQ_coeff):
        """
        Compute backward recurrence coefficients: O_ik, X_Ik, Y_Ik, and Z_Ik.
        """
        _I_Nk = self._I_Nk                # Index of penultimate junction in each superlink
        ...
        _mu_Ik = WQ_coeff._mu_Ik
        _eta_Ik = self._eta_Ik
        _O_ik = WQ_coeff._O_ik                # Recurrence coefficient O_ik
        _X_Ik = WQ_coeff._X_Ik                # Recurrence coefficient X_Ik
        _Y_Ik = self._Y_Ik                # Recurrence coefficient Y_Ik
        _Z_Ik = WQ_coeff._Z_Ik                # Recurrence coefficient Z_Ik
        NK = self.NK
        nk = self.nk
        numba_backward_recurrence_2nd(_O_ik, _X_Ik, _Y_Ik, _Z_Ik, _alpha_ik, _beta_ik, _chi_ik,
                                  _gamma_ik, _kappa_Ik, _lambda_Ik, _mu_Ik, _eta_Ik,
                                  NK, nk, _I_Nk, _i_nk)
        # Export instance variables
        self._O_ik = _O_ik
    ...
    def boundary_coefficients_2nd(self, WQ_coeff):
        _V_Ik = self._V_Ik                # Recurrence coefficient V_Ik
        _Y_Ik = self._Y_Ik                # Recurrence coefficient Y_Ik
        _eta_Ik = self._eta_Ik
        _I_1k = self._I_1k                # Index of first junction in each superlink
        ...
        _Y_uk = self._Y_uk
        _V_dk = self._V_dk
        _alpha_dk = WQ_coeff._alpha_dk
        _kappa_Ik = WQ_coeff._kappa_Ik
        _mu_Ik = WQ_coeff._mu_Ik
        _chi_uk = WQ_coeff._chi_uk
        _D_k_star = WQ_coeff._D_k_star
        _gamma_uk = self._gamma_uk
        _gamma_dk = self._gamma_dk
        _zeta_uk = self._zeta_uk
        _zeta_dk = self._zeta_dk
        self._X_uk = WQ_coeff._X_uk
        self._Z_uk = WQ_coeff._Z_uk
        self._U_dk = WQ_coeff._U_dk
        self._W_dk = WQ_coeff._W_dk
        self._theta_uk = WQ_coeff._theta_uk
        self._theta_dk = WQ_coeff._theta_dk
        # Compute boundary coefficients
        numba_boundary_coefficients_2nd(_Y_uk, _V_dk, _Y_Ik, _V_Ik, _kappa_Ik, _mu_Ik, _eta_Ik,
                                    NK, _I_1k, _I_Nk)
        # Compute boundary advection/diffusion coefficients
        _xi_uk = xi_uk(_gamma_uk, _chi_uk)
        _xi_dk = xi_dk(_gamma_dk, _alpha_dk)
        # Compute boundary transport coefficients
        _zeta_uk = zeta_uk(self._Z_uk, _V_dk, self._U_dk, _Y_uk, self._X_uk, self._W_dk,
                             self._theta_dk, _xi_uk, _xi_dk, _D_k_star)
        _zeta_dk = zeta_dk(self._W_dk, _Y_uk, self._X_uk, _V_dk, self._Z_uk,self. _U_dk,
                             self._theta_uk, _xi_uk, _xi_dk, _D_k_star)
        # Export instance variables
        self._sigma_uk = WQ_coeff._sigma_uk
        self._sigma_dk = WQ_coeff._sigma_dk
        self._rho_dk = WQ_coeff._rho_dk
        self._tau_dk = WQ_coeff._tau_dk
        self._rho_uk = WQ_coeff._rho_uk
        self._tau_uk = WQ_coeff._tau_uk
    ...
    def sparse_matrix_equations(self, c_bc=None, _Q_0j=None, _c_0j=None, u=None, _dt=None,
        ...
        _Q_uk_up_next = self._Q_uk_up_next
        _Q_dk_dn_next = self._Q_dk_dn_next
        _Q_0j_out = self.Q_0j_out   ### added for intake
        _K_j = self._K_j
        NK = self.NK
    ...
    def sparse_matrix_equations_2nd(self, WQ_coeff, c_bc=None, _Q_0j=None, _c_0j=None, u=None, _dt=None,
                                implicit=True, first_time=False):
        """
        Construct sparse matrices A, O, W, P and b.
        """
        ...
        _dx_dk = self._dx_dk
        _c_j = self._c_j
        _V_j_next = WQ_coeff._V_j_next
        _A_uk_next = WQ_coeff._A_uk_next
        _A_dk_next = WQ_coeff._A_dk_next
        _Q_uk_up_next = WQ_coeff._Q_uk_up_next
        _Q_dk_dn_next = WQ_coeff._Q_dk_dn_next
        NK = self.NK
    ...
    def step(self, dt=None, c_bc=None, c_0j=None, Q_0j=None, c_0Ik=None,
        ...
        if dt is None:
            dt = self._dt
        if Q_0j is None:
            _Q_0j = self.hydraulics._Q_in
            Q_0j = np.where(_Q_0j < 0, 0, _Q_0j)      ### if there are intake flows(negative), it should be zero here.
            self.Q_0j_out = np.where(_Q_0j < 0, -_Q_0j, 0)      ### added for intake flow rate
        if Q_0Ik is None:
    ...
    def step_2nd(self, WQ_coeff, dt=None, c_bc=None, c_0j=None, Q_0j=None, c_0Ik=None,
             Q_0Ik=None):
        """

        """
        self.import_hydraulic_states_2nd(WQ_coeff, _dt=dt)
        if dt is None:
            dt = self._dt
        if Q_0j is None:
            _Q_0j = self.hydraulics._Q_in
        ...
        if Q_0Ik is None:
            Q_0Ik = self.hydraulics._Q_0Ik
        self.node_quantities()
        self.link_coeffs_2nd(WQ_coeff, _dt=dt)
        self.node_coeffs_2nd(WQ_coeff, _Q_0Ik=Q_0Ik, _c_0Ik=c_0Ik, _dt=dt)
        self.forward_recurrence_2nd(WQ_coeff)
        self.backward_recurrence_2nd(WQ_coeff)
        self.boundary_coefficients_2nd(WQ_coeff)
        self.sparse_matrix_equations_2nd(WQ_coeff, c_bc=c_bc, _Q_0j=Q_0j,
                                     _c_0j=c_0j, _dt=dt)
        if self.hydraulics.banded:
            self.solve_banded_matrix()
        else:
...
@njit
def numba_node_coeffs_2nd(_eta_Ik, _h_Ik_next, _h_Ik_prev, _c_Ik_prev, _c_0Ik, _Q_0Ik, _A_SIk, _K_Ik,
                          _is_start, _is_end, _kI, _dt):
    N = _kI.size
    for I in range(N):
        if _is_start[I]:
...
@njit
def numba_forward_recurrence_2nd(_T_ik, _U_Ik, _V_Ik, _W_Ik, _alpha_ik, _beta_ik, _chi_ik,
                             _gamma_ik, _kappa_Ik, _lambda_Ik, _mu_Ik, _eta_Ik,
                             NK, nk, _I_1k, _i_1k):
    for k in range(NK):
        # Start at junction 1
...
@njit
def numba_backward_recurrence_2nd(_O_ik, _X_Ik, _Y_Ik, _Z_Ik, _alpha_ik, _beta_ik, _chi_ik,
                              _gamma_ik, _kappa_Ik, _lambda_Ik, _mu_Ik, _eta_Ik,
                              NK, nk, _I_Nk, _i_nk):
    for k in range(NK):
        _I_N = _I_Nk[k]
...
@njit
def numba_boundary_coefficients_2nd(_Y_uk, _V_dk, _Y_Ik, _V_Ik, _kappa_Ik, _mu_Ik, _eta_Ik,
                            NK, _I_1k, _I_Nk):
    for k in range(NK):
        _I_1 = _I_1k[k]
        _I_N = _I_Nk[k]
...
def numba_create_A_matrix(A, _F_jj, bc, _J_uk, _J_dk, _rho_uk, _rho_dk, _tau_uk, _tau_dk,
                          ...
                          _K_j, _D_uk, _D_dk, _A_uk_next, _A_dk_next, _dx_uk, _dx_dk, _Q_0j_out, M, NK):
    ...
    _F_jj += (2 * _V_j_next - _V_j_prev) / _dt + (_K_j * _V_j_next) + _Q_0j_out
